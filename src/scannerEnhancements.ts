...
function getCanvasBackgroundColor(source: HTMLCanvasElement) {
  const context = source.getContext("2d", { willReadFrequently: true });
  if (!context) return "#fff";

  const points = [
    [0, 0],
    [Math.max(0, source.width - 1), 0],
    [0, Math.max(0, source.height - 1)],
    [Math.max(0, source.width - 1), Math.max(0, source.height - 1)],
  ];
  ...
  return totalLuminance / points.length < 128 ? "#000" : "#fff";
}

export function createRotatedFrame(
  ...
  context.fillStyle = getCanvasBackgroundColor(source);
